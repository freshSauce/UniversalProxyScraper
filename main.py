...
def __argsBuilder():
    ...
    parser = argparse.ArgumentParser(
        description='Command-line option for the Universal Scraper'
    )
...
class ProxyScraper:
    ...
    def __saveFile(self, proxy_list: list) -> None:
        """
        Parameters
        -----------
        proxy_list : list
    ...
    def __doRequest(self, url: str) -> Union[bool, dict]:
        """
        Parameters
        ----------
        url : str
        ...
        try:
            result = request.request(
                'GET', url, timeout=1.5, headers=self.headers
            )
    ...
    def __poolCreator(self, urls: list[str]) -> list[Union[bool, dict]]:
        """
        Params
        ------
        None : 
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            thread_list = [
                executor.submit(self.__doRequest, url) for url in urls
            ]
    ...
    def __scrape(self, html: HTML, site: str, quantity: int) -> list[str]:
        ...
        if 'https://spys.one/en' == site:
            ...
            try:
                ...
                raise InternalError(
                    "Whoops! Seems like something wrong happened. Please, open an Issue including the URLs you're trying to scrape!"
                )
            ...
            variables = variables_raw.lstrip('(').rstrip(')').replace(
                '\\u005e', '^'
            ).split(',')

            p, r, o, x, y, s = variables[0].replace("'", ''), variables[1], variables[2], variables[3], variables[4], \
                               variables[5]

            for row in table_rows[2:]:
                if len(proxy_list) == floor(quantity * 1.5):
                    break
                ...
                ip, port = ip_port_splited[0].replace('&nbsp;',
                                                      ''), ip_port_splited[1]
        ...
        for row in table_rows:
            ...
            if 'decode' in ip or 'decode' in port:
                ...
                if not bool(
                    re.
                    search(r'[\d]{0,3}\.[\d]{0,3}\.[\d]{0,3}\.[\d]{0,3}', ip)
                ):
                    try:
    ...
    def __checker(self, proxy: str) -> bool:
        ...
        try:
            ...
            urllib3.exceptions.ProxyError, urllib3.exceptions.MaxRetryError
        ):
            return {"live": False, "proxy": proxy}

    def Proxies(self, quantity: int = 10) -> list[str]:
        """
        Parameters
        ----------
        ...
        list[str] :
            List with every proxy scraped.
        """
        urls = self.__openFile() if type(self.input) == "list" else self.input
...
if __name__ == '__main__':
    ...
    while True:
        result = ProxyScraper(args['file'], args['output'],
                              args['verify']).Proxies(args['quantity'])
        ...
        while True:
            option = input(
                'Everything is done !!! Wanna get more proxies? (Y[es]/N[o]): '
            ).lower()
